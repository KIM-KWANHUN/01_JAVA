    ...
    public static void main(String[] args) {
        ...
        *   하지만 내부적으로 요소를 저장하는 방식이 다르다는 것은 인지해야 한다!! */
        ...
        for(int i = 0; i < linkedList.size(); i++) {
            System.out.println(i + "번 째의 값 : " + linkedList.get(i)); // 순서를 유지한다.
        }
        ...
        linkedList.set(0,"pineapple"); // 값 수정 0번째 인덱스에 있는 apple 를 pineapple 로 수정함.
        System.out.println("linkedList = " + linkedList);

        // 리스트 내 모든 요소를 제거하는 메소드
        linkedList.clear();
        // isEmpty() -> 비어있는 지 확인하는 메소드 true, false 로 나옴
