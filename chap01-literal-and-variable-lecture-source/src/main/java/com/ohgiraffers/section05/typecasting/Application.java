...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        *  - ex) int - int, char - char emdemd
        *  - ** 즉, 같은 데이터 타입끼리만 연산이 가능하다. */
        /* comment.
        *   형변환의 종류와 규칙
        *   1. 자동형변환(묵시적 형변환) : 컴파일러가 자동으로 수행해주는 변환
        *       1-1. 작은 자료형에서 큰 자료형으로 자동으로 형변환 된다.
        *       1-2. 정수는 실수로 자동 형변환 된다.
        *       1-3. 문자형은 int 형으로 자동 형변환 된다.(문자는 숫자이다)
        *       1-4. 논리형은 형변환 규칙에서 제외 된다.
        *   2. 강제형변환(명시적 형변환) : 형변환 연산자 ex) (long), (int)
        *                              를 이용한 강제적으로 수행하는 변환
        *       2-1 자동이 안되면 강제로 해야한다.
        *    */

        /* index. 1. 자동형변환 규칙 테스트 */
        /* index 1-1. 작은 자료형에서 큰 자료형으로는 자동변환 된다.
        ...
        *    데이터의 유실은 발생하지 않기 때문에 컴파일러가 자동으로 해준다. */
        ...
        // 자동으로 큰 쪽인 long 으로 변경 후 계산하기 때문에 int형 변수 공간에 담을 수 없다.
        ...
        // float result2 = fnum; + dnum; 마찬가지로 에러가 뜬다.
        
        /* index. 1-2 정수는 실수로 자동형변환이 된다. */
        /* comment. 정수에서 실수로 변경할 때 소수점이 없어도 실수 형태로 표현이 가능하다.
        *   그냥 소수점만 추가 되는것이기 때문에 ex) 30, 30.0 */
        
        long eight = 8; //8byte
        long four = eight; //4byte
