...
public class KindOfVariable { //클래스 영역의 시작
    ...
    /* comment.
    *   클래스 영역 내부에 작성하는 변수를 필됴 라고 한다.
    *   필드 == 멤버변수(클래스가 가진 멤버) == 전역변수 */

    /* comment.
    *   non-static 필드를 인스턴스 변수 라고도 한다.
    *   인스턴스 생성 시점에 사용 가능한 변수의 의미 */
    private int globalNum;

    /* comment.
    *   static 필드는 정적필드(클래스 변수) 라고도 한다. */
    ...
    public void testMethod(int num) { // 메소드 영역의 시작
        ...
        *   stack 영역에 생성이 된다. */
        ...
        // 지역변수는 선언 외에 다시 사용하기 위해서는 반드시 초기화를 해야 한다.
       // System.out.println(localNum);
        System.out.println(globalNum);
        System.out.println(staticNum); // 전역변수는 메소드 안에 사용할 수 있다.
    ...
    public void testMethod2() {
        ...
        // System.out.println(localNum); 지역변수는 다른 공간에 사용할 수 없다.
